...
5. Work Experience Format: Format EVERY bullet point using this exact structure: "[Focus Area]: [Action verb-led sentence with impact and quantification]". DO NOT include bullet characters (like • or ·) in the JSON.Rewrite using the Problem → Action → Result (XYZ) framework. Focus on execution and process improvement rather than basic tasks. Replace passive phrases with strong action verbs. You MUST inject metric placeholders (e.g., 'improving efficiency by [X]%', 'managing [X] client accounts') if exact numbers aren't provided. Format as: "[Focus Area]: [Action verb-led sentence]". DO NOT include bullet characters.
...
5. Work Experience Format: Format EVERY bullet point using this exact structure: "[Focus Area]: [Action verb-led sentence with impact and quantification]". DO NOT include bullet characters (like • or ·) in the JSON. Rewrite using the Problem → Action → Result (XYZ) framework. Focus heavily on strategic ownership, business impact, and leadership rather than basic, daily tasks. Replace passive phrases with strong executive action verbs (e.g., Spearheaded, Architected, Directed). You MUST inject high-level metric placeholders (e.g., 'generating $[X]M in revenue', 'scaling team by [X]%', 'reducing costs by $[X]') if exact numbers are missing to force measurable achievements. Format as: "[Focus Area]: [Action verb-led sentence]". DO NOT include bullet characters.`;
    }
